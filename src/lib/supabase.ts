...
/**
 * Helper to convert a File or Blob into a Base64 Data URL for server-side transport
 */
...
/**
 * Upload gift image to Supabase Storage via the secure server-side route (/api/admin/upload-gift-image).
 * The server uses SUPABASE_SERVICE_ROLE_KEY to safely bypass Storage RLS without exposing keys.
 * Returns the public URL generated by Supabase Storage.
 */
export async function uploadGiftImageToSupabaseStorage(
  ...
  try {
    const dataUrl = await fileOrBlobToDataUrl(fileOrBlob);
    const contentType = fileOrBlob.type || 'image/webp';
    const cleanFileName = originalFileName || `gift_${giftId}_${index}.webp`;
    ...
    const response = await fetch('/api/admin/upload-gift-image', {
      method: 'POST',
      headers,
      body: JSON.stringify({
        image: dataUrl,
    ...
    const errorMessage = result?.error || `Falha no upload (status ${response.status}: ${response.statusText})`;
    return {
      url: '',
      error: errorMessage,
    };
  } catch (err: any) {
    console.error('Erro na chamada da rota server-side de upload:', err);
    return {
      url: '',
      error: `Erro de conexão ao enviar imagem para o servidor: ${err?.message || 'Falha de comunicação'}`,
    };
  }
}
